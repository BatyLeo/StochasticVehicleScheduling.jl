...
function (model::MyModel)(x)
    y = vec(model.dense(x.features))
    return model.fwrp(y; linear_maximizer_kwargs=(; instance=x))
end
...
#pipeline(x) = easy_problem(model(x), instance=x);

initial_pred = [model(x) for x in X_test]
...
@showprogress for epoch in 1:nb_epochs
    l = mean(flux_loss(x) for x in X_train)
    l_test = mean(flux_loss(x) for x in X_test)
    # l = mean(flux_loss(x, y) for (x, y) in data_train)
    # l_test = mean(flux_loss(x, y) for (x, y) in data_test)
    Y_pred = [easy_problem(model(x); instance=x) for x in  X_test]
    ...
    #@info "Info" epoch l l_test V
    Flux.train!(flux_loss, Flux.params(model), zip(X_train), opt)
end

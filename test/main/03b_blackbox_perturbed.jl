...
loss = PerturbedComposition(PerturbedAdditive(trainer.pipeline.maximizer; ε=1000, M=5, seed=0), cost)
...
res1 = bboptimize(blackbox_loss; SearchRange=(-1.0, 1.0), NumDimensions=dim, MaxTime=60*30)
...
res2 = bboptimize(direct_blackbox_loss; SearchRange=(-1.0, 1.0), NumDimensions=dim, MaxTime=60*30)

best_fitness(res2)
w2 = best_candidate(res2)
final_encoder2 = Chain(Dense(reshape(w2[1:nb_features], 1, nb_features), w2[nb_features+1:end]), vec)
value2 = mean(evaluate_solution(easy_problem(final_encoder2(x.features); instance=x), x) for x in X)

train_cost = [evaluate_solution(easy_problem(final_encoder2(x.features); instance=x), x) for x in X];
train_cost_gap = mean(
    (c - c_opt) / abs(c_opt) for (c, c_opt) in zip(train_cost, train_cost_opt)
)

test_cost = [evaluate_solution(easy_problem(final_encoder2(x.features); instance=x), x) for x in X_test];
test_cost_gap = mean(
    (c - c_opt) / abs(c_opt) for (c, c_opt) in zip(test_cost, test_cost_opt)
)

struct InferOptTrainer{M1<:Tuple, M2<:Tuple, P, L, I}
    train_metrics::M1
    test_metrics::M2
    pipeline::P
    loss::L
...
function InferOptTrainer(; metrics_dict::Dict, loss, pipeline, extra_info)
    train_metrics = Tuple(metric("Train $name") for (name, metric) in metrics_dict)
    test_metrics = Tuple(metric("Test $name") for (name, metric) in metrics_dict)
